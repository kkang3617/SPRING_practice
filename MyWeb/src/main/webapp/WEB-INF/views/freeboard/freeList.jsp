		    ...
		    <form action="${pageContext.request.contextPath}/freeboard/freeList" name="pageForm">
                    ...
                    <ul id="pagination" class="pagination pagination-sm">
                        ...
                        <li class="${pc.paging.pageNum == num ? 'active' : ''}"> 
	                        <!-- 사용자가 선택한페이지번호가 num과 같을때만 active를 넣어라 -->
...
<script>
    ...
    window.onload = function() {
        ...
        //기존에는 각각의 a태그의 href에 각각 다른 url을 작성해서 보내줬다면,
        //이번엔 클릭한 그 버튼이 무엇인지를 확인해서 그 버튼에 맞는 페이지 정보를
        //자바스크립트로 끌고와서 요청을 보내주겠다(submit).
        document.getElementById('pagination').addEventListener('click', e => { //e 누가 눌렸는지 알기위함.
            if(!e.target.matches('a')) { //이벤트 발생주체가 a이벤트가 아니면 
                return; // 강제종료
            }

            e.preventDefault(); //a태그의 고유 기능 중지.

            //현재 이벤트 가 발생한 요소(버튼)의
