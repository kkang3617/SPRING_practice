    ...
    <script>
        ...
        window.onload = function () {
            ...
            document.getElementById('replyRegist').onclick = () => {
                ...
                const replyPw = document.getElementByPw('replyPw').value;

                if(reply === '' || replyId === '' || replyPw === '') {
                    alert('이름, 비밀번호, 내용 입력하세요!');
                    return;
                }
            ...
            const reqObj = {
                method: 'post',
                headers: {
                    'Content-Type':'application/json' //text/plain은 한개만 보낼 때.
                },
                body: JSON.stringify({  //아래의 객체를 JSON형태로 바꾸겠다.
                    'bno' : bno,
                    'reply' : reply,
                    'replyId' : replyId,
                    'replyPw' : replyPw
            ...
            fetch('${pageContext.request.contextPath}/reply/regist', reqObj)
                ...
                //요청 페이지의 번호와, 화면을 리셋할 것인지의 여부를 bool 타입의 reset으로 받겠습니다.
