...
<script>
    window.onload = function () {
        ...
        document.getElementById('moreList').onclick = () => {
            /*
            왜 false를 주느냐?
            더보기이기때문, 댓글을 누적해서 보여줘야하지 않을까요?
            1페이지의 댓글 내용 밑에다가 2페이를 누적해서 깔아야 한다.
            1페이지 내용을 없애고 2페이지를 보여주자는 것이 아니기 때문..
            */
            getList(++page, false);
        }
        ...
        보장이 없기 때문에 해당 방식은 등록이 불가능하다.
        이 때는, 이미 실제로 존재하는 #replyList에 등록하고, 이벤트를 자식에게 위임하여
        사용하는 addEventListener를 통해 처리해야 한다.
        */
        ...
        document.getElementById('replyList').addEventListener('click', e => {
            ...
            const content = e.target.parentNode.nextElementSibling.textContent; // nextelementsibling 다음형제요소
            console.log('댓글 내용: ' + content);                               //parentNode 윗부모
        ...
        //수정 처리 함수. (수정 모달을 열어서 수정 내용을 작성 후 수정 버튼을 클릭했)
            ...
            fetch('${pageContext.request.contextPath}/reply/' + rno, reqObj)
                ...
                .then(data => {
                    if(data === 'pwFail') {
                        ...
                        document.getElementById('modalPw').value.focus();
        ...
        document.getElementById('modalDelBtn').onclick = () => {
            ...
            */
            ...
            fetch('${pageContext.request.contextPath}/reply/' + rno, reqObj)
                .then(res => res.text())  // 결과(res)를 text로 받는다. 아래 if문
                .then(data => {
                    if(data === 'pwFail') {
                        alert('비밀번호를 확인하세요.');
                        document.getElementById('modalPw').value = ''; //다시써야해서 공백으로 비워주고
                        document.getElementById('modalPw').value.focus(); //커서를 위치하게 해줌.
                        ...
                        //제이쿼리 문법으로 bootstrap 모달 닫아주기
                        $('#replyModal').modal('hide');
                        getList(1, true); //게시글 상세보기 화면에 처음 진입했을 시 댓글 리스트를 한 번 불러오자.
                    }
            ...
            //댓글 날짜 변환 함수
            function parseTime(regDateTime) { //regDateTime 시간정보가 담겨져있는 숫자배열이 온다
                let year, month, day, hour, minute, second;

                if(regDateTime.length === 5) { //날짜 배열 5냐?
                ...
                const date = new Date();
                console.log(date);
                const gap = date.getTime() - regTime.getTime(); //현재시간-등록시간

                let time;
                if(gap < 60 * 60 * 24 * 1000) { // 1000 -> 밀리초
                    if(gap < 60 * 60  * 1000) {  //한시간이 안 넘었다면
                        time = '방금 전';
                    } else {
                        time = parseInt(gap / (1000 * 60 * 60)) + '시간 전'; //parseInt 정수로 변환
                    }
                } else if(gap < 60 * 60 * 24 * 30 * 1000) { // 30일이 안 지났으면
                    time = parseInt(gap / (60 * 60 * 24 * 1000)) + '일 전';
                } else {
