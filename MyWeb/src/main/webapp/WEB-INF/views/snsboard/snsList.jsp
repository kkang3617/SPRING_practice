...
<body>
	...
	<script>
		...
		function regist() {
			...
			const userId = '${sessionScope.login}'; //sessionScope는 안써도됨 안쓰면 자동으로 login 찾아줌.
			...
			if(file !== 'jpg' && file !== 'png' && file !== 'jpeg' && file !== 'bmp') {
				alert('이미지 파일(jpg, png, jpeg, bmp) 만 가능합니다.');
				document.getElementById('file').value = ''; //비우고
				return; //강종
			...
			FormData 객체를 활용한다.
			무조건 이 방식이 좋은 것은 아니다. FromData는 비동기 방식이 꼭 필요한 경우에만 사용한다.
			대부분의 경우에는 form 태그를 이용해서 전송하는 방식이 더 간편한고 더 자주 사용된다.
			*/
			const formData = new FormData();
			const $data = document.getElementById('file'); // 이미지 첨부 input

			console.log('data: ' + $data);
			//한 화면에 파일 업로드 버튼이 여러개일 경우 요소의 인덱스를 지목해서 가져올 수 있음.
			//우리는 파일 첨부 버틍니 하나고, id를 지목해서 가져오기 때문에 인덱스를 쓸 필요는 없다.
			...
			//files -> FileList에 여러 정보가 들어오게 된다.
			//FileList는 유사 배열이기 때문에 인덱스를 이용해서 여러 파일 중 하나를 지목할 수 있다.
			//우리는 지금 multiple이 없기 때문에 [0]이리고 작성하면 파일 정보를 얻을 수 있다.
        ...
        //서버쪽에서 권한을 확인 해 주세요. (작성자와 로그인 중인 사용자의 id를 비교해서 일치하는지의 여부) - 모르겠음..
        //일치하지 않는다면 문자열 "noAuth" 리턴, 삭제 완료하면 "success" 리턴
        //url: /snsboard/글번호 method: DELETE

		document.getElementById('contentDiv').onclick = e => {
			e.preventDefault();
