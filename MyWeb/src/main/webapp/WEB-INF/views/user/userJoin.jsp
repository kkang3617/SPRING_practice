...
<%@ include file="../include/footer.jsp" %>
    ...
    <script>
    	...
    	//아이디 중복 체크
    	document.getElementById('idCheckBtn').onclick = function() { //아이디 중복 버튼 클릭한다면
            ...
            /* 틀린코드 아님 아래 fetch API 쓰기 위해 주석처리

            //아이디 중복확인 비동기 요청 준비
            const xhr = new XMLHttpRequest(); //비동기요청준비할 수 있게 해주는 객체

            //서버 요청 정보 설정
            xhr.open('POST', '${pageContext.request.contextPath}/user/idCheck');
            //요청 정보를 헤더에 설정
            xhr.setRequestHeader('content-type', 'text/plain'); //객체가 아닌 문자 하나(userId)만 보내는 거라 text/plain 사용
            xhr.send(userId);
            ...
            */
            ...
            //Promise 객체의 상태가 요청 성공일 시 데이터 후송 처리진행(then함수 실행하겠다.)
            .then(res => {
                //fetch 함수를 통해 비동시 통신이 실행되고,
        ...
        document.getElementById('mail-check-btn').onclick = function() {
            ...
            fetch('${pageContext.request.contextPath}/user/mailCheck?email=' + email) // get방식이라 객체 보낼 필요 x body method header 등등
                .then(res => res.text())
                	.then(data => {
        ...
        document.querySelector('.mail-check-input').onblur = function(e) {
            ...
            if(inputCode === code) {
                ...
                e.target.focus(); //다시 입력할 수 있게 포커싱 주기
            } 
        }// 인증번호 검증 끝.

        //다음 주소 api 사용해 보기 (script src도 추가해야 한다.)
    ...
    document.getElementById('joinBtn').onclick = function() {
        ...
        if(idFlag && pwFlag) {
            if(!document.getElementById('userId').getAttribute('readonly')) { //readonly가 안걸려있니?
        ...
        id.onkeyup = function() { //눌렀다 뗄 때 
            /*
            	자바스크립트의 정규표현식 입니다
            	정규표현식(RegularExpression): 문자열 내의 특정 문자 조합을 찾기 위한 패턴이다.
            	특정 규칙이 있는 문자열 집합을 규칙 직접 지정하여 탐색하게 해주는 
            	메타 문자 이다.
            */
