...
public class FreeBoardControllerTest {
	...
	/*
	 ...
	 - WebAllpicationContext는 Spring에서 제공되는 servlet들을 사용할 수 있도록
	 정보를 저장하는 객체이다.
	 */
	@Autowired
...
//	private FreeBoardController controller;
	...
	@BeforeEach //테스트 시작 다른 메서드 실행전에 항상 먼저 구동되는 기능들을 선언.
	public void setup() {
		//가상 구조를 세팅
...
//		this.mockMvc = MockMvcBuilders.standaloneSetup(controller).build();
	...
	void testList() throws Exception {
		ModelAndView mv = 
			...
			//mockmvc 동작해라, freeboard/freelist 요청을 보냄
			.andReturn()
			.getModelAndView();
	...
	void testContent() throws Exception {
		ModelAndView mv =
				mockMvc.perform(MockMvcRequestBuilders.get("/freeboard/content?bno=3"))
				//.param("bno", "3")
				.andReturn()
				.getModelAndView();
